...
impl CurrentMeter {
    ...
    pub fn receive(&mut self, data: &mut dyn Iterator<Item = u32>) {
        match self.state {
    ...
    fn calibrate(&mut self, data: &mut dyn Iterator<Item = u32>) {
        for d in data {
            let d = d as i32;
            self.square_sum += d;
            self.count += 1;
    ...
    fn process(&mut self, data: &mut dyn Iterator<Item = u32>) {
        for d in data {
            let d = d as i32;
            ...
            match self.state {
                ...
                State::Active(prev_over) => {
                    ...
                    if prev_over && under {
                        ...
                        self.stats.store(rms_ma as u32, std::sync::atomic::Ordering::Relaxed);
